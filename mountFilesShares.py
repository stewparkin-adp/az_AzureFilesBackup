...
def get_storage_accounts(token,
 subscription_id):
...
def mount_and_add_to_fstab(account_name,
 share_name,
 client_id,
 client_secret):
 ...
 0o600)
    ...
    cmd_mount = f"sudo mount -t cifs //{account_name}.file.core.windows.net/{share_name} {mount_path} -o vers=3.0,
credentials={credentials_path},
sec=ntlmssp"
    subprocess.run(cmd_mount,
 shell=True)

    fstab_entry = f"//{account_name}.file.core.windows.net/{share_name} {mount_path} cifs vers=3.0,
credentials={credentials_path},
sec=ntlmssp 0 0\n"
    with open('/etc/fstab',
 'a') as f:
...
def main(args):
 ...
 args.client_secret,
 args.resource)
    storage_accounts = get_storage_accounts(token,
 ...
 0o700)
    ...
    for account in storage_accounts:
        print (account)
        shares = get_file_shares(token,
 account,
 args.subscription_id)
        for share in shares:
            mount_and_add_to_fstab(account['name'],
 share['name'],
 args.client_id,
 args.client_secret)
...
if __name__ == "__main__":
 ...
 help="Azure Resource Endpoint (default: https://management.azure.com/)")
